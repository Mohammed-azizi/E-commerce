...
function product_exists($pid){
    ...
    for($i=0;$i<$max;$i++){
      if($pid==$_SESSION['gcCart'][$i]['productid']){
        $flag=1;
        message("المنتج موجود في سلة التسوق","error");
        break;
      }
    }
    return $flag;
    ...
    if(is_array($_SESSION['gcCart'])){
     ...
     $_SESSION['gcCart']=array();
      ...
      $_SESSION['gcCart'][0]['price']=$price;
         ...
         message(" .تمت إضافة منتج في عربة التسوق","success");
}

function product_existsFr($pid){
  $pid=intval($pid);
  $max=count($_SESSION['gcCart']);
  $flag=0;
  for($i=0;$i<$max;$i++){
...
function addtocartFr($pid,$q,$price){
  if($pid<1 or $q<1) return;
  if (!empty($_SESSION['gcCart'])){


  if(is_array($_SESSION['gcCart'])){
    if(product_existsFr($pid)) return;
    $max=count($_SESSION['gcCart']);
    $_SESSION['gcCart'][$max]['productid']=$pid;
    $_SESSION['gcCart'][$max]['qty']=$q;
    $_SESSION['gcCart'][$max]['price']=$price;
